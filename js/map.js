...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!map) {
    ...
    // Add tile layer
    L.tileLayer(
      "https://tiles.stadiamaps.com/tiles/stamen_terrain/{z}/{x}/{y}{r}.png?api_key=22685591-9232-45c7-a495-cfdf0e81ab86",
      {
        maxZoom: 18,
        attribution:
    ...
    legend.onAdd = function (map) {
      ...
      div.innerHTML =
        `<h4>Neighbourhood Rank<br><small>(1 = Best)</small></h4>` +
        labels.join("<br>");
      return div;
    };
    ...
    .then((data) => {
      ...
      if (data && data.type === "FeatureCollection") {
        ...
        const sorted = data.features.sort(
          (a, b) => a.properties.rank_am_np_1000 - b.properties.rank_am_np_1000,
        );
        ...
        const maxRank = Math.max(
          ...data.features.map((f) => f.properties.rank_am_np_1000),
        );
        top5 = data.features
          .sort(
            (a, b) =>
              a.properties.rank_am_np_1000 - b.properties.rank_am_np_1000,
          )
          .slice(0, 5)
          ...
          onEachFeature: function (feature, layer) {
            ...
            layer.bindTooltip(
              feature.properties.AREA_NAME +
                " (Rank: " +
                feature.properties.rank_am_np_1000 +
                ")",
              {
                permanent: false,
                direction: "top",
...
function populateDropdown(data) {
  ...
  dropdown.addEventListener("change", () => {
    ...
    if (!isNaN(selectedId)) {
      const match = data.features.find((f) => f.properties._id === selectedId);
...
function resetBottom6() {
  geoJsonLayer.eachLayer((layer) => {
    const name = layer.feature.properties.AREA_NAME;
    if (bottom6.includes(name)) {
      layer.isBottom6 = false; // unset flag
      geoJsonLayer.resetStyle(layer);
    }
  });
}
